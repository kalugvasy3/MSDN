...
namespace WpfHelperCopyData
{
    ...
    /// </summary>
    public partial class DataSetResult : Window
    {
        ...
        public void runSQL(string strProviderNameFrom, string strConnectionString, StringBuilder sbRun)
        {
            ...
            try
            {
                ...
                Action MainThreadLoadSQL = new Action(() =>
                {
                    ...
                    try
                    {
                        ...
                        {
                            ds = db.GetDataSet(ref sbRun, strProviderNameFrom, strConnectionString, out strError, out intEffected);  
                            dsResult = ds;
        ...
        private void csvSaveAs(object sender, KeyEventArgs e)
        {
            ...
            if (e.Key == Key.S && Keyboard.Modifiers == ModifierKeys.Control)
            {
                ...
                foreach (DataRow row in dTable[intT].Rows)
                {
                    ...
                    for (int i = 0; i < intClmn; i++)
                    {
                        sb.Append((row[i].ToString().Replace(',',' ').Replace(System.Environment.NewLine, "; ") + (i == intClmn - 1 ? " " : ",")));
                    }
                    streamWriter.WriteLine(sb.ToString());
                }
                streamWriter.Flush();
